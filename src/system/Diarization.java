...
//		return filter(clusterSetFrom, clusterSetSNS, parameter);
	...
	private ClusterSet filter(ClusterSet clusterSetFrom, ClusterSet clusterSetSNS) throws DiarizationException{
		...
		if(clusterSetSNS.getCluster("S") != null){
...
//		GMMArrayList gmmInitVect = new GMMArrayList(2);
	...
	public ClusterSet runEHMM(ClusterSet clusterSet, Parameter parameter, AudioFeatureSet featureSet) throws Exception, IOException{
		/* 加载ubm */
		InputStream ubmInputStream = null;
		GMMArrayList ubmVect = null;
...
//			System.err.println("use UBM for 8KHz");
		...
		GMM ubm = ubmVect.get(0);
		return EHMM.EHMMProcess(clusterSet, featureSet, ubm, parameter);
	}
	/**
	 * called
	 * 程序主体函数 Diarization.
	...
	public void diarization(Parameter parameter, ClusterSet clusterSet)
		...
		if(this.logFileWriter == null)
			...
			this.logFileWriter.write(date + "\tStarting Segmentation...\t10% completed\n");
		...
		if(this.logFileWriter == null)
			...
			this.logFileWriter.write(date+"\tFinishing Segementation\t20% completed\n");
			this.logFileWriter.flush();
		}
		...
		if(clusterNoSilence.getClusterVectorRepresentation().size() == 0 || 
				(clusterNoSilence.getClusterVectorRepresentation().size() == 1 && clusterNoSilence.getCluster("S") == null)){
			if(this.logFileWriter == null)
				System.err.println(date+"\t this is a non-speech audio, system will exit");
			else {
				this.logFileWriter.write(date+"\t this is a non-speech audio, system will exit\n");
				this.logFileWriter.flush();
			}
			...
			if(this.logFileWriter == null)
				...
				this.logFileWriter.write(date+"\tFinishing removing silence\t30% completed\n");
				this.logFileWriter.flush();
			}
		}
		//音频小于3min，直接处理；3min以上，只取前三分钟
...
//		}
		...
		//线性聚类。进行粗聚，把相邻的segment合并
		ClusterSet clustersLClust = clusteringLinear(0.0, clusterNoSilence, featureSet, parameter);
		date = new Date();
		if(this.logFileWriter == null)
			System.err.println(date+"\tFinishing linear clustering\t40% completed");
		else{
			this.logFileWriter.write(date+"\tFinishing linear clustering\t40% completed\n");
			this.logFileWriter.flush();
		}
//		ClusterSet clustersDClust = runEHMM(clustersLClust, parameter, featureSet);
		...
		if(this.logFileWriter == null)
			...
			this.logFileWriter.write(date+"\tFinishing hierarchical clustering\t90% completed\n");
			this.logFileWriter.flush();
		}
		...
		if(this.logFileWriter == null)
			...
			this.logFileWriter.write(date+"\tFinishing decoding\t100% completed\n");
			this.logFileWriter.flush();
		}
...
//		}
	...
	private ClusterSet removeLittleSegment(ClusterSet clustersDClust) {
		...
		while(iterator.hasNext()){
			...
			if(segment.getLength() < 30){//30
				Cluster cluster = segment.getCluster();
				cluster.removeSegment(segment);
				iterator.remove();
			}
		}
		for(Cluster cluster : clustersDClust.getClusterVectorRepresentation()){
			if(cluster.segmentsSize() == 0){
				clustersDClust.removeCluster(cluster.getName());
			}
		}
		return clustersDClust;
	}
...
//		fos.close();
	...
	public static Parameter getParameter(String[] args) {
		...
		if(parameter.sampleRate.equals("6")){
			...
			parameter.getParameterInputFeature().setFeaturesDescription(
					"audio16kHz2sphinx,1:1:0:0:0:0,13,0:0:0:0");
		}
		return parameter;
	}
	...
	public static int make(String[] args){
		try {
			...
			app.arguments = args;
			Parameter parameter = getParameter(args);//读参数
			if (args.length <= 1) {
				parameter.help = true;
			} 
			if(parameter.outlogFile != null && !parameter.outlogFile.equals("")){
				...
				app.logFileWriter = new FileWriter(parameter.outlogFile, true);
			}
			...
			if(isWaveFormat(inputFile)){//判断格式
			...
			Date date = new Date();
			if(app.logFileWriter == null)
				System.err.println(date+"\tDiarization tuning\t 0% completed");
			else{
				app.logFileWriter.write(date+"\tDiarization tuning\t 0% completed\n");
			...
			if(app.logFileWriter == null)
				...
				app.logFileWriter.write(date+"\tDone! Please check " + segOutFilename +" for result\n");
				app.logFileWriter.flush();
			}
			...
			BufferedReader br = new BufferedReader(new InputStreamReader(new FileInputStream(segFile.getAbsoluteFile()), 
													"UTF-8"));
			String temp;
